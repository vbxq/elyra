        {
            ...
            match inner_opcode {
                ...
                OpCode::LoadI => {
                    let immediate_bits =
                        u16::try_from(b).expect("wide immediate occupies two bytes");
                ...
                | OpCode::GeFFG => {
                    ...
                    let result = match inner_opcode {
                        ...
                        | OpCode::LtFF
                        | OpCode::LtIIG
                        | OpCode::LtFFG => self.compare_lt(left, right)?,
                        OpCode::Le
                        | OpCode::LeII
                        | OpCode::LeFF
                        | OpCode::LeIIG
                        | OpCode::LeFFG => self.compare_le(left, right)?,
                        OpCode::Gt
                        | OpCode::GtII
                        | OpCode::GtFF
                        | OpCode::GtIIG
                        | OpCode::GtFFG => self.compare_gt(left, right)?,
                ...
                | OpCode::ArrayLoadP => {
                    ...
                    let value = match self.heap.get(array_ref) {
                        ...
                        None => {
                            return Err(
                                self.runtime_error(RuntimeErrorKind::InvalidMemoryHandle)
                            );
                        }
                    };
                    reg_set!(base + a, value);
                ...
                | OpCode::ArrayGetP => {
                    ...
                    let value = usize::try_from(index)
                        ...
                        .and_then(|index| {
                            let array_ref = GcRef::new(array_value.as_ptr().unwrap_or(0));
                            self.heap.get(array_ref).and_then(|object| match &object.kind {
                    ...
                    let loaded = match self.heap.get(vec_ref) {
                        Some(object) => match &object.kind {
                            ...
                            ObjectKind::String(string) if inner_opcode == OpCode::VecLoadP => {
                                let character = string.as_str().chars().nth(index_usize).ok_or_else(
                                    || {
                                        self.runtime_error(RuntimeErrorKind::IndexOutOfBounds {
                                            index,
                                            length: i64::try_from(
                ...
                OpCode::VecGetI | OpCode::VecGetF | OpCode::VecGetB | OpCode::VecGetP => {
                    ...
                    let value = usize::try_from(index)
                        ...
                        .and_then(|index| {
                            let vec_ref = GcRef::new(vec_value.as_ptr().unwrap_or(0));
                            self.heap.get(vec_ref).and_then(|object| match &object.kind {
                ...
                OpCode::GetUpval => {
                    if b >= upvalues_len {
                        return Err(self.runtime_error(RuntimeErrorKind::UndefinedVariable(
                            format!("upvalue index {b} out of bounds"),
                        )));
                    }
                ...
                OpCode::SetUpval => {
                    if a >= upvalues_len {
                        return Err(self.runtime_error(RuntimeErrorKind::UndefinedVariable(
                            format!("upvalue index {a} out of bounds"),
                        )));
                    }
