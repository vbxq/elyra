...
pub(crate) fn execute(
    ...
    match opcode {
        ...
        24 => {
            ...
            let name = global_name(vm, state, index, "GetGlobal")?;
            state.save_ip(vm);
            let value = vm
                .get_global(&name)
                .ok_or_else(|| vm.runtime_error(RuntimeErrorKind::UndefinedVariable(name)))?;
            state.write_register(vm, state.base + usize::from(destination), value)?;
        }
        // SetGlobal (25)
        25 => {
            let (source, immediate) = decode_aimm(instruction);
            let index = usize::from(u16::from_ne_bytes(immediate.to_ne_bytes()));
            let name = global_name(vm, state, index, "SetGlobal")?;
            let value = state.read_register(vm, state.base + usize::from(source))?;
            state.save_ip(vm);
